...
<div class="cont">
  <section class="header flex text-sm">
    <div class="font-bold text-lg">hutang / detail hutang #{id}</div>
    <div class="flex-grow"></div>
    <a href={`/app/ap/${id}/add_payment`} class="primary">
      tambah pembayaran
    </a>
  </section>
  <section class="bg-white w-1/2 mr-4 card-detail">
    ...
    <ul>
      <li>
        <div>Supplier</div>
        <div>
          <a href={`/app/customer/${supplier.id}/edit`}>{supplier.first_name}</a>
        </div>
      </li>
      <li>
